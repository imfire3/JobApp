/**
 * Capture crisp dark-mode screenshots for the landing page.
 * Requires: npm run dev on :3000
 *
 * Usage: node scripts/capture-landing-screens.mjs
 */
import { chromium } from "playwright"
import fs from "fs"
import path from "path"

const BASE = "http://localhost:3000"
const OUT = path.join(process.cwd(), "public", "landing")
const DOCS = path.join(process.cwd(), "docs", "screenshots")

const CV = `Vincent Giacalone
Product Manager / Product Owner

Expérience
Product Owner — Onboarding & Conversion, Fortuneo (2024 – 2025)
- Augmentation de 9 % de la conversion d'ouverture de compte grâce à l'optimisation du funnel d'onboarding.
- Pilotage backlog, ateliers de découverte et coordination de livraison.

Product Owner — Growth & Acquisition, Choisir by Impala (2022 – 2024)
- Acquisition, parcours utilisateur et A/B testing.

Compétences: Product Management, Roadmap, Agile, SQL, Figma, A/B testing
Formation: Master Management
`

const COVER = `Madame, Monsieur,

Je vous contacte concernant le poste de Product Owner. Mon parcours chez Fortuneo et Choisir by Impala m'a permis de piloter des roadmaps, d'animer des ateliers de découverte et de coordonner la livraison avec design et engineering.

Chez Fortuneo, j'ai contribué à une hausse de 9 % de la conversion d'ouverture de compte en optimisant le funnel d'onboarding. Je suis motivé à apporter cette approche produit, centrée preuves et résultats, à votre équipe.

Je serais ravi d'échanger sur la mission.

Cordialement,
Vincent Giacalone
`

async function dismiss(page) {
  for (const label of [
    "Plus tard",
    "J’ai compris",
    "J'ai compris",
    "Passer",
    "Skip",
    "Close",
    "Fermer",
  ]) {
    const btn = page.getByRole("button", { name: new RegExp(label, "i") })
    if (await btn.count()) {
      await btn.first().click({ timeout: 800 }).catch(() => {})
      await page.waitForTimeout(250)
    }
  }
}

async function gotoApp(page, url) {
  await page.goto(`${BASE}${url}`, {
    waitUntil: "domcontentloaded",
    timeout: 60000,
  })
  await page.waitForTimeout(1200)
  await dismiss(page)
}

async function hideChrome(page) {
  await page.addStyleTag({
    content: `
      nextjs-portal, [data-nextjs-toast], [data-sonner-toaster] { display: none !important; }
    `,
  })
  await page.evaluate(() => {
    document.documentElement.classList.add("dark")
    document.documentElement.classList.remove("light")
    localStorage.setItem("theme", "dark")
    for (const el of document.querySelectorAll("button, a, span, div")) {
      const t = (el.textContent || "").replace(/\s+/g, " ").trim()
      if (t === "Reset → LP" || t === "Reset →LP") {
        el.style.setProperty("display", "none", "important")
      }
    }
  })
}

async function save(page, name) {
  await hideChrome(page)
  await page.waitForTimeout(350)
  const file = path.join(OUT, name)
  await page.screenshot({ path: file, fullPage: false, type: "png" })
  fs.copyFileSync(file, path.join(DOCS, name))
  console.log("saved", name, fs.statSync(file).size)
}

async function main() {
  fs.mkdirSync(OUT, { recursive: true })
  fs.mkdirSync(DOCS, { recursive: true })

  const stamp = Date.now().toString(36)
  const email = `shot+${stamp}@jobapp.local`
  const password = "password1"

  const browser = await chromium.launch({ headless: true })
  const context = await browser.newContext({
    viewport: { width: 1600, height: 1000 },
    deviceScaleFactor: 2,
    colorScheme: "dark",
  })
  const page = await context.newPage()
  await page.addInitScript(() => {
    localStorage.setItem("theme", "dark")
    document.documentElement.classList.add("dark")
  })

  const signup = await context.request.post(`${BASE}/api/auth/signup`, {
    data: { identifier: email, password },
  })
  console.log("signup", signup.status())

  const login = await context.request.post(`${BASE}/api/auth/login`, {
    data: { identifier: email, password },
  })
  console.log("login", login.status())

  const profile = await context.request.put(`${BASE}/api/profile`, {
    data: {
      cv_text: CV,
      first_name: "Vincent",
      last_name: "Giacalone",
      current_title: "Product Manager / Product Owner",
      skills: ["Product Management", "Agile", "SQL", "Figma"],
      target_roles: ["Product Owner", "Product Manager"],
      target_locations: ["Paris"],
      profile_reviewed: true,
    },
  })
  console.log("profile", profile.status())

  const onboard = await context.request.patch(`${BASE}/api/onboarding`, {
    data: { completed: true },
  })
  console.log("onboarding", onboard.status(), (await onboard.text()).slice(0, 120))

  await gotoApp(page, "/dashboard")
  console.log("url dashboard", page.url())
  if (page.url().includes("/login")) {
    throw new Error("Still on login after auth — aborting")
  }

  let seed = await context.request.post(`${BASE}/api/dev/seed-jobs`, {
    data: { count: 16 },
    headers: { "content-type": "application/json" },
  })
  let seedBody = await seed.text()
  console.log("seed", seed.status(), seedBody.slice(0, 180))
  if (seed.status() !== 200 || seedBody.trim().startsWith("<!")) {
    seed = await context.request.post(`${BASE}/api/import-jobs/sample`)
    seedBody = await seed.text()
    console.log("sample", seed.status(), seedBody.slice(0, 180))
  }

  await page.reload({ waitUntil: "domcontentloaded" })
  await page.waitForTimeout(1000)
  await dismiss(page)
  await save(page, "screen-dashboard.png")

  await gotoApp(page, "/jobs")
  await page.waitForTimeout(600)
  await save(page, "screen-jobs.png")

  await gotoApp(page, "/imports")
  const csvTab = page.getByRole("button", { name: /CSV\s*\/\s*Excel/i })
  if (await csvTab.count()) await csvTab.first().click().catch(() => {})
  await page.waitForTimeout(400)
  await save(page, "screen-imports.png")

  await gotoApp(page, "/extension")
  await save(page, "screen-extension.png")

  await gotoApp(page, "/profile-ai")
  await page.waitForTimeout(500)
  await save(page, "screen-profile.png")

  const jobsRes = await context.request.get(`${BASE}/api/jobs`)
  const jobsJson = await jobsRes.json()
  const jobs = jobsJson.jobs || []
  const jobId = jobs[0]?.id
  console.log("jobId", jobId, "count", jobs.length)

  if (jobId) {
    const patch = await context.request.patch(`${BASE}/api/jobs`, {
      data: { id: jobId, cover_letter: COVER },
    })
    console.log("patch letter", patch.status())

    await context.request
      .post(`${BASE}/api/applications`, {
        data: {
          company: jobs[0].company || "Alan",
          position: jobs[0].title || "Product Owner",
          status: "hr_interview",
          notes: "Super",
          job_id: jobId,
        },
      })
      .catch(async (e) => console.log("application create skip", e?.message))

    await gotoApp(page, `/jobs/${jobId}`)
    await page.waitForTimeout(1000)
    const overview = page.getByRole("tab", { name: /Vue d'ensemble|Overview/i })
    if (await overview.count()) await overview.first().click().catch(() => {})
    await page.waitForTimeout(600)
    await save(page, "screen-job-detail.png")

    const optTab = page.getByRole("tab", { name: /Optimiser/i })
    if (await optTab.count()) {
      await optTab.first().click()
      await page.waitForTimeout(1200)
      await save(page, "screen-optimize.png")
    } else {
      await gotoApp(page, "/profile-ai/optimize")
      await save(page, "screen-optimize.png")
    }

    // Re-open job for letter
    await gotoApp(page, `/jobs/${jobId}`)
    const cand = page.getByRole("tab", { name: /Candidature/i })
    if (await cand.count()) {
      await cand.first().click()
      await page.waitForTimeout(800)
    }
    const letterBtn = page.getByRole("button", {
      name: /lettre|letter|Voir la lettre|Générer|Afficher/i,
    })
    if (await letterBtn.count()) {
      await letterBtn.first().click().catch(() => {})
      await page.waitForTimeout(1200)
    }
    await save(page, "screen-cover-letter.png")
  } else {
    console.warn("No jobs — keeping previous detail/optimize/letter screens")
  }

  await gotoApp(page, "/applications")
  await page.waitForTimeout(800)
  await save(page, "screen-applications.png")

  await browser.close()
  console.log("done")
}

main().catch((err) => {
  console.error(err)
  process.exit(1)
})
