...
export function buildActorInput(options: JobConnectorOptions): Record<string, unknown> {
  const search = options.trackedSearch
  const remoteTypes = mapRemoteForActor(search.remote_preference)
  ...
  return {
    ...
    remoteTypes,
    contractTypes: mapContractTypesForActor(search.contract_types ?? []),
    minimumSalary: search.minimum_salary,
    maximumSalary: search.maximum_salary ?? undefined,
    salaryMin: search.minimum_salary,
    salaryMax: search.maximum_salary ?? undefined,
    salaryPeriod: search.salary_period ?? "year",
    currency: search.currency,
    languages: search.languages ?? [],
    categories: search.expertises ?? [],
    expertises: search.expertises ?? [],
    companyNames: search.company_names ?? [],
    onlyWithSalary: search.only_with_salary ?? false,
    exclusiveOnly: search.exclusive_only ?? false,
    topRecruiterOnly: search.top_recruiter_only ?? false,
    startDatePreference: search.start_date_preference ?? undefined,
    datePosted: search.publish_window ?? undefined,
    websiteCountry: "fr",
    countryCode: "FR",
  }
}
