...
async function loadOnboardingState(
  ...
  const [
    ...
    supabase
      .from("profiles")
      .select("target_roles,target_locations,profile_reviewed_at")
      .eq("id", userId)
      .maybeSingle(),
    supabase
      .from("cv_analyses")
  ...
  const targetLocations = Array.isArray(profile?.target_locations)
  ...
  const hasProfileReviewed =
    typeof profile?.profile_reviewed_at === "string" &&
    profile.profile_reviewed_at.length > 0
...
function buildOnboardingResponse(flags: LoadedOnboardingState) {
  return {
    ...
    has_cv: flags.hasCv,
    has_profile_reviewed: flags.hasProfileReviewed,
...
export async function GET() {
  ...
  try {
    let flags = await loadOnboardingState(supabase, user.id)

    // Clés API are optional: once CV + profile are ready, mark onboarding done.
    if (!flags.completed && canCompleteOnboarding(flags)) {
  ...
  try {
    ...
    if (!body.completed || !canCompleteOnboarding(flags)) {
      return NextResponse.json(
        { error: "Onboarding cannot be completed yet" },
        { status: 400 }
      )
    }
